...
import { json } from "@remix-run/node"; // Đảm bảo rằng bạn đã nhập json từ Remix
...
// Hàm lấy các bài tập phù hợp với tình trạng sức khỏe của người dùng
const getRecommendedExercises = async (userHealthStatus) => {
  const {
    targetMuscleGroups,
    equipmentPreference,
    intensityPreference,
    fitnessGoals,
    condition, // condition là một chuỗi đơn giản từ bảng HealthStatus
  } = userHealthStatus;
  ...
  try {
    const exercises = await prisma.exercise.findMany({
      where: {
        AND: [
          // Kiểm tra nhóm cơ mục tiêu nếu có
          targetMuscleGroups && targetMuscleGroups.length > 0
            ? {
                target: {
                  in: targetMuscleGroups, // Lọc bài tập theo nhóm cơ mục tiêu
                },
              }
            : null,

          // Kiểm tra dụng cụ nếu có
          equipmentPreference && equipmentPreference.length > 0
            ? {
                equipment: {
                  in: equipmentPreference, // Lọc bài tập theo dụng cụ
                },
              }
            : null,

          // Kiểm tra mức độ cường độ nếu có
          intensityPreference
            ? {
                intensity: intensityPreference, // Lọc bài tập theo cường độ
              }
            : null,

          // Kiểm tra điều kiện sức khỏe nếu có
          condition
            ? {
                healthConditions: {
                  hasSome: [condition], // Kiểm tra xem mảng healthConditions có chứa condition hay không
                },
              }
            : null,
        ].filter(Boolean), // Loại bỏ các điều kiện không có giá trị
      },
    });
    ...
    throw new Error("Could not fetch recommended exercises");
  }
};

// Loader để lấy thông tin tình trạng sức khỏe của người dùng
export async function loader({ request }) {
  // Import authenticator chỉ khi cần sử dụng
  ...
  // Lấy session từ người dùng và lưu userId vào session nếu chưa có
  ...
  if (!healthStatus) {
    return json({ error: "Health status not found" }, { status: 404 });
  }

  const exercises = await getRecommendedExercises(healthStatus);

  // Xóa tất cả các bài tập cũ của người dùng này trong bảng Workout
  await prisma.workout.deleteMany({
    where: { userId: existingUser.id },
  });

  // Lưu các bài tập mới vào bảng Workout
  const workoutPromises = exercises.map((exercise) =>
  ...
  // Đợi tất cả các thao tác tạo workout hoàn thành
  await Promise.all(workoutPromises);
  const allReviews = getAllReviews(exercises);

  return json({ user, healthStatus, exercises: allReviews });
}
...
export default function HealthStatusForm() {
  const { user, healthStatus, exercises, error } = useLoaderData();
  ...
  // Nếu không có tình trạng sức khỏe
  if (!healthStatus) {
    return <p>Không tìm thấy thông tin tình trạng sức khỏe.</p>;
  }
    ...
    <>
      ...
      <div className="recommended-workout">
        ...
        <div className="health-status">
          <h3>Health Status</h3>
          <p>
            <strong>Weight:</strong> {healthStatus.weight} kg
          </p>
          <p>
            <strong>Height:</strong> {healthStatus.height} cm
          </p>
          <p>
            <strong>Age:</strong> {healthStatus.age} years
          </p>
          <p>
            <strong>Health Condition:</strong> {healthStatus.condition}
          </p>
          <p>
            <strong>Fitness Goals:</strong> {healthStatus.fitnessGoals}
          </p>
        </div>
        ...
        <div className="recommended-exercises">
          ...
          <ul className="card_group">
            {allReviews.slice(startIndex, endIndex).map((exercise) => (
              <li
                ...
                onClick={() => handleNavigateToDetail(exercise.id)}
              >
                <div className="relative w-full bg-white rounded-xl overflow-hidden hover: transform hover:scale-103 transition-transform duration-200 ease-in-out">
                  {/* Workout Duration (outside the image) */}
                  <div className="absolute top-2 left-2 z-20">
                    <div className="bg-green-500 text-white text-xs font-medium px-3 py-1 rounded-md">
          ...
          {allReviews.length === 0 && (
            <p className="no-exercises">No suitable exercises available.</p>
          )}
