...
import "../styles/workoutcustom.css";
// Loader function để lấy danh sách Workout Plans và thông tin chi tiết bài tập từ bảng Exercise
...
export async function loader({ request }) {
  try {
    ...
    // Thiết lập session nếu chưa có userId
    ...
    const workoutPlans = await prisma.workoutPlan.findMany({
      where: { userId: userId }, // Lọc theo userId nếu cần
      include: {
        exercise: {
          select: {
            name: true,
    ...
    return json({ user, workoutPlans });
  } catch (error) {
    // Log lỗi và trả về thông tin
...
function groupByDays(workoutPlans) {
  return workoutPlans.reduce((acc, plan) => {
    const days = plan.days.split(", "); // Tách ngày thành mảng
    days.forEach((day) => {
      if (!acc[day]) {
        acc[day] = []; // Khởi tạo mảng cho ngày nếu chưa có
      }
      acc[day].push(plan); // Thêm kế hoạch tập luyện vào ngày tương ứng
    });
    return acc;
  }, {});
}
...
export default function WorkoutPlans() {
  const { user, workoutPlans } = useLoaderData();
  const groupedPlans = groupByDays(workoutPlans);
  ...
  // Hàm điều hướng đến trang chi tiết bài tập
  const handleNavigateToDetail = (workoutId) => {
    setSelectedWorkoutId(workoutId);
    navigate(`/ExerciseDetaill/?pI=${workoutId}`);
  };
  const {
    isOpen: isOpen2,
    onOpen: onOpen2,
  ...
  const handleEdit = (workout) => {
    ...
    onOpen2(); // Mở modal khi người dùng nhấn Edit
  };
  const handleChange = (field, value) => {
    setSelectedWorkout((prev) => ({ ...prev, [field]: value }));
  };
  ...
  //     { method: "post", action: "/deleteAndeditapi" }
  //   );
  // };

  // const handleEdit = (id, updates) => {
  //   fetcher.submit(
  //     { actionType: "edit", id, updates: JSON.stringify(updates) },
  //     { method: "post", action: "/deleteAndeditapi" }
  //   );
  const handleUpdateworkout = async (e) => {
    ...
    const response = await fetch("/editworkoutapi", {
      method: "POST",
      ...
      <div className="workout-plan-list">
        ...
        <Accordion variant="splitted" className="workout-plan-accordion">
          {Object.keys(groupedPlans).length > 0 ? (
                ...
                <ul className="plan-list">
                  {groupedPlans[day].map((plan) => (
                    <li
                      key={plan.id}
                      className="plan-item"
                      onClick={() => handleNavigateToDetail(plan.workoutId)}
                    >
                      <div className="plan-header">
                        <h4 className="plan-name">{plan.exerciseName}</h4>
                                ...
                                {(onClose) => (
                                  <>
                                    <ModalHeader className="flex flex-col gap-1">
                                      edit {selectedWorkout.exerciseName}
                                    </ModalHeader>
                                    <ModalBody>
                                      <Form
                                        method="post"
                                        action="/editworkoutapi"
                                      ...
                                      >
                                        ...
                                        <label>
                                          ...
                                          <input
                                            type="number"
                                            name="time"
                                            defaultValue={selectedWorkout.time}
                                          />
                                        </label>
                                        <label>
                                          Sets:
                                          <input
                                            type="number"
                                            name="sets"
                                            defaultValue={selectedWorkout.sets}
                                          />
                                        </label>
                                        <label>
                                          Reps:
                                          <input
                                            type="number"
                                            name="reps"
                                            defaultValue={selectedWorkout.reps}
                                          />
                                        </label>
                                        <ModalFooter>
                                          <Button
                            ...
                            >
                              <ModalContent>
                                {(onClose) => (
                                  <>
                                    <ModalHeader className="flex flex-col gap-1">
                                      Delete {selectedWorkout.exerciseName}
                                    </ModalHeader>
                                    <ModalBody>
                                      <p>
                                      ...
                                      <Form
                                        method="post"
                                        action="/deleteapi"
                                        onSubmit={handleDeletefrom}
                                        className="delete_from"
                                      >
                                        <input
                                          type="hidden"
                                          name="Id"
                            ...
                            <ul>
                              <li>
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    handleNavigateToDetail(plan.workoutId)
                                  }
                                >
                                  {plan.exercise.name}
                                </button>
                              ...
                              <li>
                                ...
                                {plan.exercise.secondaryMuscles.join(", ")}
                              </li>
                              <li>
                              ...
                              <li>
                                <strong>GIF: </strong>
                                <a href={plan.exercise.gifUrl} target="_blank">
                                  View
                                </a>
                              </li>
                            </ul>
            ...
            <AccordionItem key="no-plans" title="No Plans Available">
              <p>
                No workout plans available. Add some from the Workout Plan page.
              </p>
            </AccordionItem>
          )}
        </Accordion>
