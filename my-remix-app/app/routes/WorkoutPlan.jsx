...
// Loader để lấy tất cả các bài tập và bài tập từ bảng workout
export async function loader({ request }) {
  ...
  // const allExercises = await prisma.exercise.findMany();
  const recommendedExercises = await prisma.workout.findMany();

  return json({ user, allExercises, recommendedExercises });
}

export default function WorkoutPlan() {
  const { user, allExercises, recommendedExercises } = useLoaderData();
  ...
  const exercisesToDisplay =
    selectedSource === "all" ? allExercises : recommendedExercises;
    ...
    setSelectedExercises((prevSelected) =>
      prevSelected.map((exercise, i) =>
        i === index
          ? {
              ...exercise,
              days: exercise.days.includes(day)
                ? exercise.days.filter((d) => d !== day)
                : [...exercise.days, day],
            }
          : exercise
      )
    ...
    <>
      ...
      <div className="workout-plan">
        <div className="header">
          ...
          <h2 className="page-title">Customize Your Workout Plan</h2>
        </div>
        ...
        <div className="workout-container">
          ...
          <div className="selected-exercises">
            ...
            {selectedExercises.length > 0 ? (
              selectedExercises.map((exercise, index) => (
                <div key={exercise.id} className="exercise-details">
                  ...
                  <div className="details-input">
                    <label>
                      <i className="icon-clock"></i> Time (min):
                      <input
                        type="number"
                        value={exercise.time}
                        onChange={(e) =>
                    ...
                    <label>
                      <i className="icon-barbell"></i> Sets:
                      <input
                        type="number"
                        value={exercise.sets}
                        onChange={(e) =>
                    ...
                    <label>
                      <i className="icon-repeat"></i> Reps:
                      <input
                        type="number"
                        value={exercise.reps}
                        onChange={(e) =>
                    ...
                    <div className="days-group">
                      {daysOfWeek.map((day) => (
                        <label key={day}>
                          <input
                            type="checkbox"
                            checked={exercise.days.includes(day)}
                            onChange={() => handleDaySelection(index, day)}
                          />
                          {day === "Monday" && <FontAwesomeIcon icon={faSun} />}
                          {day === "Tuesday" && (
              ...
              <p>No exercises selected. Start customizing your plan!</p>
            )}
          </div>
        </div>

        <Form method="post" action="/Workoutplanapi" className="submit-form">
          <input
            type="hidden"
